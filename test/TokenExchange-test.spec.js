...
!developmentChains.includes(network.name)
  ? describe.skip
  : describe('TokenExchange Contract Tests', () => {
      ...
      let deployer, TokenExchange;
      let TETHER, SUSHI, USDC, WETH;

      beforeEach(async () => {
        [deployer] = await ethers.getSigners();

        const tokenExchangeFactory = await ethers.getContractFactory('TokenExchange', deployer);
        TokenExchange = await tokenExchangeFactory.deploy(UNISWAP_V2_ROUTER);
        await TokenExchange.deployed();

        await TokenExchange.connect(deployer);
      ...
      it('Should swap tokens', async () => {
        ...
        await WETH.connect(deployer).approve(TokenExchange.address, amountIn);
        await TokenExchange.connect(deployer).exchangeTokens(
          tokenIn,
          tokenOut,
          amountIn,
          AMOUNT_OUT_MIN,
        ...
        await USDC.connect(deployer).approve(TokenExchange.address, amountUsdcTether);
        ...
        const usdtSwap = await TokenExchange.connect(deployer).exchangeTokens(
          USDC_ADDRESS,
          TETHER_ADDRESS,
          amountUsdcTether,
          AMOUNT_OUT_MIN,
